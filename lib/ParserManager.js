...
class ParserManager {
    ...
    setOptions(options) {
        ...
        // If the options contain a 'type', then make sure that the options are
        // passed down to the correct parser type as well.
        if (this.options && this.options.type &&
            typeof this.parsers[this.options.type].setOptions === 'function') {
